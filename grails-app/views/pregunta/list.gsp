			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="order" title="${message(code: 'pregunta.order.label', default: 'Order')}" />
				...
				<g:each in="${preguntaInstanceList}" status="i" var="preguntaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${preguntaInstance.id}">${fieldValue(bean: preguntaInstance, field: "order")}</g:link></td>
