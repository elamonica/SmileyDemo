	...
	<body>
		...
		<div id="list-carModel" class="content scaffold-list" role="main">
			...
			<table>
				<thead>
					<tr>
						...
						<th><g:message code="carModel.carBrand.label" default="Car Brand" /></th>
					
						<g:sortableColumn property="name" title="${message(code: 'carModel.name.label', default: 'Name')}" />
				...
				<g:each in="${carModelInstanceList}" status="i" var="carModelInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${carModelInstance.id}">${fieldValue(bean: carModelInstance, field: "carBrand")}</g:link></td>
					
						<td>${fieldValue(bean: carModelInstance, field: "name")}</td>
			...
			<div class="pagination">
				<g:paginate total="${carModelInstanceTotal}" />
			</div>
		</div>
	</body>
</html>
